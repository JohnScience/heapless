...
///
/// // A vector with a fixed capacity of 8 elements allocated on the stack
/// let mut vec = AlignedVec::<_, A256, 8>::new();
/// vec.push(1);
/// vec.push(2);
///
/// assert_eq!(vec.len(), 2);
...
impl<T, A, const N: usize> GenericVec<T, A, N> {
    ...
    ///
    /// // allocate the vector on the stack
    /// let mut x: AlignedVec<u8, A256, 16> = AlignedVec::new();
    ///
    /// // allocate the vector in a static variable
    /// static mut X: AlignedVec<u8, A256, 16> = AlignedVec::new();
    /// ```
    ...
    /// Constructs a new aligned vector with a fixed capacity of `N` and fills it
    ...
    /// struct A256;
    ///
    /// let mut v: AlignedVec<u8, A256, 16> = AlignedVec::new();
    ...
    /// struct A256;
    ///
    /// let buffer: AlignedVec<u8, A256, 5> = AlignedVec::from_slice(&[1, 2, 3, 5, 8]).unwrap();
    /// assert_eq!(buffer.as_slice(), &[1, 2, 3, 5, 8]);
    /// ```
    ...
    /// of the vector is exactly `M`, otherwise returns `Err(self)`. Note that this
    /// array is no longer guaranteed to be aligned.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// struct A256;
    ///
    /// let buffer: AlignedVec<u8, A256, 42> = AlignedVec::from_slice(&[1, 2, 3, 5, 8]).unwrap();
    /// // NOTE: the array may be unaligned
    /// let array: [u8; 5] = buffer.into_array().unwrap();
    /// assert_eq!(array, [1, 2, 3, 5, 8]);
    /// ```
    ...
    /// struct A256;
    ///
    /// let mut buffer: AlignedVec<u8, A256, 5> = AlignedVec::from_slice(&[1, 2, 3, 5, 8]).unwrap();
    /// buffer[0] = 9;
    /// assert_eq!(buffer.as_slice(), &[9, 2, 3, 5, 8]);
    /// ```
    ...
    /// Clones and appends all elements in a slice to the `AlignedVec`.
    ///
    /// Iterates over the slice `other`, clones each element, and then appends
    /// it to this `AlignedVec`. The `other` vector is traversed in-order.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// struct A256;
    ///
    /// let mut vec = AlignedVec::<u8, A256, 8>::new();
    /// vec.push(1).unwrap();
    /// vec.extend_from_slice(&[2, 3, 4]).unwrap();
    /// assert_eq!(*vec, [1, 2, 3, 4]);
    /// ```
        ...
        T: Clone,
    {
        if self.len + other.len() > self.capacity() {
            // won't fit in the `AlignedVec`; don't modify anything and return an error
            Err(())
        } else {
            for elem in other {
                unsafe {
    ...
    /// Resizes the `AlignedVec` in-place so that `len` is equal to `new_len`.
    ///
    /// If `new_len` is greater than `len`, the `Vec` is extended by the
    ...
    /// # }
    /// # impl StreamWrapper {
    /// pub fn get_dictionary(&self) -> Option<AlignedVec<u8, A256, 32768>> {
    ///     // Per the FFI method's docs, "32768 bytes is always enough".
    ///     let mut dict = AlignedVec::new();
    ///     let mut dict_length = 0;
    ///     // SAFETY: When `deflateGetDictionary` returns `Z_OK`, it holds that:
    ///     // 1. `dict_length` elements were initialized.
    ///     // 2. `dict_length` <= the capacity (32_768)
    ...
    /// struct A256;
    ///
    /// let mut vec = AlignedVec::<AlignedVec<u8, A256, 3>, A256, 3>::from_iter(
    ///     [
    ///         AlignedVec::from_iter([1, 0, 0].iter().cloned()),
    ///         AlignedVec::from_iter([0, 1, 0].iter().cloned()),
    ///         AlignedVec::from_iter([0, 0, 1].iter().cloned()),
    ///     ]
    ///     .iter()
    ///     .cloned(),
    /// );
    ...
    /// struct A256;
    ///
    /// let mut v: AlignedVec<_, A256, 8> = AlignedVec::new();
    /// v.push("foo").unwrap();
    /// v.push("bar").unwrap();
    /// v.push("baz").unwrap();
    /// v.push("qux").unwrap();
    ...
    /// Returns `true` if `needle` is a prefix of the AlignedVec.
    ///
    ...
    /// struct A256;
    ///
    /// let v: AlignedVec<_, A256, 8> = AlignedVec::from_slice(b"abc").unwrap();
    /// assert_eq!(v.starts_with(b""), true);
    /// assert_eq!(v.starts_with(b"ab"), true);
    /// assert_eq!(v.starts_with(b"bc"), false);
    /// ```
    ...
    /// Returns `true` if `needle` is a suffix of the AlignedVec.
    ///
    /// Always returns `true` if `needle` is an empty slice.
    ///
    /// # Examples
    ...
    /// struct A256;
    ///
    /// let mut vec: AlignedVec<_, A256, 8> = AlignedVec::from_slice(&[1, 2, 3]).unwrap();
    /// vec.insert(1, 4);
    /// assert_eq!(vec, [1, 4, 2, 3]);
    /// vec.insert(4, 5);
    /// assert_eq!(vec, [1, 4, 2, 3, 5]);
    ...
    /// struct A256;
    ///
    /// let mut v: AlignedVec<_, A256, 8> = AlignedVec::from_slice(&[1, 2, 3]).unwrap();
    /// assert_eq!(v.remove(1), 2);
    /// assert_eq!(v, [1, 3]);
    /// ```
    ...
    /// struct A256;
    ///
    /// let mut vec: AlignedVec<_, A256, 8> = AlignedVec::from_slice(&[1, 2, 3, 4]).unwrap();
    /// vec.retain(|&x| x % 2 == 0);
    /// assert_eq!(vec, [2, 4]);
    /// ```
    ///
    ...
    /// struct A256;
    ///
    /// let mut vec: AlignedVec<_, A256, 8> = AlignedVec::from_slice(&[1, 2, 3, 4, 5]).unwrap();
    /// let keep = [false, true, true, false, true];
    /// let mut iter = keep.iter();
    /// vec.retain(|_| *iter.next().unwrap());
    /// assert_eq!(vec, [2, 3, 5]);
        ...
        F: FnMut(&mut T) -> bool,
    {
        ...
        impl<T, A, const N: usize> Drop for BackshiftOnDrop<'_, T, A, N> {
            fn drop(&mut self) {
        ...
        fn process_loop<F, T, A, const N: usize, const DELETED: bool>(
            original_len: usize,
            f: &mut F,
            g: &mut BackshiftOnDrop<'_, T, A, N>,
        ) where
            F: FnMut(&mut T) -> bool,
        {
        ...
        process_loop::<F, T, A, N, false>(original_len, &mut f, &mut g);

        // Stage 2: Some elements were deleted.
        process_loop::<F, T, A, N, true>(original_len, &mut f, &mut g);
...
impl<T, A, const N: usize> Iterator for IntoIter<T, A, N> {
    type Item = T;
...
impl<T, A, const N: usize> Clone for IntoIter<T, A, N>
where
    T: Clone,
{
    fn clone(&self) -> Self {
...
impl<T, A, const N: usize> Drop for IntoIter<T, A, N> {
    fn drop(&mut self) {
        unsafe {
...
mod tests {
    ...
    #[test]
    fn static_new() {
        static mut _V: AlignedVec<i32, A256, 4> = AlignedVec::new();
    }

    #[test]
    fn stack_new() {
        let mut _v: AlignedVec<i32, A256, 4> = AlignedVec::new();
    }

    #[test]
    fn is_full_empty() {
        let mut v: AlignedVec<i32, A256, 4> = AlignedVec::new();
    ...
    fn drop() {
        ...
        {
            let mut v: AlignedVec<Droppable, A256, 2> = AlignedVec::new();
    ...
    #[test]
    fn eq() {
        let mut xs: AlignedVec<i32, A256, 4> = AlignedVec::new();
        let mut ys: AlignedVec<i32, A256, 8> = AlignedVec::new();
    ...
    fn cmp() {
        let mut xs: AlignedVec<i32, A256, 4> = AlignedVec::new();
        let mut ys: AlignedVec<i32, A256, 4> = AlignedVec::new();
    ...
    fn cmp_with_arrays_and_slices() {
        let mut xs: AlignedVec<i32, A256, 12> = AlignedVec::new();
        xs.push(1).unwrap();
    ...
    fn collect_from_iter() {
        let slice = &[1, 2, 3];
        let vec: AlignedVec<i32, A256, 4> = slice.iter().cloned().collect();
        assert_eq!(&vec, slice);
    }
    ...
    fn collect_from_iter_overfull() {
        let slice = &[1, 2, 3];
        let _vec = slice.iter().cloned().collect::<AlignedVec<_, A256, 2>>();
    }
    ...
    fn iter_move_drop() {
        ...
        {
            let mut vec: AlignedVec<Droppable, A256, 2> = AlignedVec::new();
    ...
    fn resize_size_limit() {
        let mut v: AlignedVec<u8, A256, 4> = AlignedVec::new();
    ...
    fn from_slice() {
        ...
        let v: AlignedVec<u8, A256, 4> = AlignedVec::from_slice(&[1, 2, 3]).unwrap();
        assert_eq!(v.len(), 3);
        assert_eq!(v.as_slice(), &[1, 2, 3]);

        // Slice too large
        assert!(AlignedVec::<u8, A256, 2>::from_slice(&[1, 2, 3]).is_err());
    }

    #[test]
    fn starts_with() {
        let v: AlignedVec<_, A256, 8> = AlignedVec::from_slice(b"ab").unwrap();
    ...
    #[test]
    fn zero_capacity() {
        let mut v: AlignedVec<u8, A256, 0> = AlignedVec::new();
        // Validate capacity
